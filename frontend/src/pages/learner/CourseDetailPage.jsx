...
import CourseHero, { LessonList } from "../../components/course-detail/CourseHero";
import ReviewSection from "../../components/course-detail/ReviewSection";
...
export default function CourseDetailPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const responseData = detailRes.data.data;
        const courseData = responseData?.course || responseData;
        setCourse(courseData);
        const enr = responseData?.enrollment || null;
        setEnrollment(enr);
        setHasInvitation(!!responseData?.hasInvitation);
        setHasPaid(!!responseData?.hasPaid);
        ...
        if (!enr && courseData?.accessRule === "ON_PAYMENT" && user) {
          try {
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      const responseData = detailRes.data.data;
      setEnrollment(responseData?.enrollment || null);
      setHasInvitation(!!responseData?.hasInvitation);
      setHasPaid(!!responseData?.hasPaid);
    } catch (err) {
      const msg =
  ...
  const handlePaypalApprove = async (data) => {
    ...
    try {
      ...
      const detailRes = await getCourseDetail(id);
      const responseData = detailRes.data.data;
      setCourse(responseData?.course || responseData);
      setEnrollment(responseData?.enrollment || null);
      setHasInvitation(!!responseData?.hasInvitation);
      setHasPaid(!!responseData?.hasPaid);
    } catch (err) {
  ...
  return (
    <div className="space-y-8">
      <CourseHero course={course} avgRating={avgRating} />
